...
const MAX_ATTEMPTS = 8; // serializable txns abort on concurrent row updates; retry is the protocol
...
async function applyDelta(
  ...
  for (let attempt = 1; ; attempt++) {
    try {
      ...
      if (attempt < MAX_ATTEMPTS && isSerializationConflict(e)) continue;
      throw e;
    }
  }
}
