...
import { ResellerCockpit } from "@/components/ResellerCockpit";
import { getResellerRateSheet, getResellerOrdersWithKeys } from "@/lib/reseller";
...
export default async function AccountPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const isReseller = user?.role === "RESELLER" && !!user?.tier;

  // If reseller, load rate sheet and delivered orders with keys
  let rateSheet: Awaited<ReturnType<typeof getResellerRateSheet>> = [];
  let resellerOrders: Awaited<ReturnType<typeof getResellerOrdersWithKeys>> = [];
  let dzdRate = 240;

  if (isReseller && user.tier) {
    const settings = await getSystemSettings();
    dzdRate = settings.dzdRate;
    [rateSheet, resellerOrders] = await Promise.all([
      getResellerRateSheet(user.tier.id),
      getResellerOrdersWithKeys(session.userId),
    ]);
  }

  // Regular orders query for standard customer view
  const regularOrders = !isReseller
  ...
  return (
    <div className={`mx-auto px-4 py-10 sm:px-6 ${isReseller ? "max-w-4xl" : "max-w-2xl"}`}>
      ...
      {/* Reseller VIP Mode vs Customer Mode */}
      {isReseller && user.tier ? (
        <div className="mt-6 flex flex-col gap-6">
          ...
          <ResellerCockpit
            ...
            tier={{
              ...
              minDepositMinor: Number(user.tier.minDepositMinor),
            }}
            rateSheet={rateSheet}
