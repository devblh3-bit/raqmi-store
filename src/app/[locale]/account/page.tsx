...
export default async function AccountPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-10 sm:px-6">
      ...
      {/* Wallet summary */}
      <div className="mt-6 flex items-center justify-between gap-4 rounded-3xl border border-[var(--border)] bg-[var(--surface)] p-6 shadow-[var(--elev-1)]">
        <div>
      ...
      {/* Logout: a plain form POST. The route clears the cookie and redirects,
          so this works without client JS and needs no fetch plumbing. */}
      <form action="/api/auth/logout" method="post" className="mt-4">
        <button className="inline-flex h-11 w-full items-center justify-center rounded-full border border-[var(--border)] bg-[var(--surface)] text-sm font-semibold shadow-sm transition-all hover:shadow-md">
          {t("signOut")}
        </button>
      </form>
    </div>
